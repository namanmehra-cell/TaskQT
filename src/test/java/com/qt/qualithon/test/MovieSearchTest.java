...
import com.qt.qualithon.rottentomatoe.MoviePageRT;
import com.qt.qualithon.rottentomatoe.WebAppRT;
...
public class MovieSearchTest {
    ...
    public void testSearchByExactMovieTitleReturnsMovieAsFirstResult(String title){
        ...
        MoviePage movieOnImdbWeb = new WebApp(this.testSession)
            ...
            .search(title)
            .firstMovieResult();
//        MoviePageRT movieOnRT = new WebAppRT(this.testSession)
//                .launch()
//                .search(title)
//                .firstMovieResult();

       assertThat(movieOnImdbWeb.title()).isEqualTo(title);
       //assertThat(movieOnRT.title()).isEqualTo(title);
    }
    ...
    public void testMovieMetadataOnWebHasCorrectMaturityRating(String title) throws Exception {
        ...
        MoviePage movieOnImdbWeb = new WebApp(this.testSession)
            ...
            // get Movie metadata from http://www.omdbapi.com/
            Movie movie = new OMDbAPI().getMovie(title);
            assertThat(movieOnImdbWeb.maturityRating()).isEqualTo(movie.maturity());
...
//                .firstMovieResult();
            // get Movie metadata from http://www.omdbapi.com/
            Movie movie = new OMDbAPI().getMovie(title);
            assertThat(movieOnImdbWeb.rating()).isEqualTo(movie.movieRating());
